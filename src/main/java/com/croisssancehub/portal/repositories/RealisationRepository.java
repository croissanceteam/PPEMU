...
public interface RealisationRepository extends JpaRepository<Realisation,Long> {
   ...
   // List<Realisation> findByDateExport(String date);
    ...
    @Query("select rep.id as idRep,rep.nameClient as nameClient, rep.lat as latitude, rep.lng as longitude,\n" +
            "rep.refClient as ClientRep,rep.avenue as avenue,rep.numHome as numHome,rep.commune as street,rep.category as category,rea.town as town,rea.entreprise as entreprise, rea.consultant as contractor,\n" +
            "rea.refClient as RefClient from Reperage  rep left join Realisation rea on rea.refClient=rep.refClient order by rep.commune")
    ...
    @Query("select rep.id as idRep,rep.nameClient as nameClient, rea.lat as latitude, rea.lng as longitude,\n" +
            "rep.refClient as ClientRep,rep.avenue as avenue,rep.numHome as numHome,rea.commune as street,rep.category as category,rea.town as town,rea.entreprise as entreprise, rea.consultant as contractor,\n" +
            "rea.refClient as RefClient from Realisation  rea left join Reperage rep on rea.refClient=rep.refClient order by rea.commune")
